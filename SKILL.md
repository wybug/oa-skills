...
**触发场景**：当用户需要操作OA系统中的通用页面（非待办审批）时使用。
...
1. **创建探索会话**（内部执行：`oa-todo explore <url> --pause --timeout 15`）
   输出包含 session ID、url、timeout、agentUXContext

2. **智能体解析 agentUXContext**
   智能体获取探索引导文件，了解页面分析框架和交互流程

3. **选择操作方式**：
   - **方式1: HTTP API 请求**（推荐用于数据查询）
     - 使用 OATools.http() 直接获取数据
     - 适用于：查询会议室、获取待办列表、数据统计
     - 示例：获取会议室占用情况、查询会议安排
   - **方式2: 浏览器操作**（用于页面交互）
     - 使用 OATools.browser() 进行页面操作
     - 适用于：填写表单、点击按钮、页面交互
     - 示例：会议室预约、请假申请、费用报销
...
5. **多轮对话理解用户意图**
   智能体引导用户描述要完成的操作，逐步收集必要信息

6. **生成命令序列并确认执行**
   智能体生成完整的 agent-browser 命令序列，用户确认后执行

7. **执行总结**
   生成包含命令序列和外部参数的总结，便于重复操作

**支持的场景**：
- **会议室查询**：使用 HTTP API 查询会议室占用情况
- **会议室预约**：使用浏览器操作填写预约表单
- **请假申请**：填写假期类型、时间、事由
- **费用报销**：填写金额、类型、事由、附件
- **通用表单**：任何OA系统内的表单填写
...
| 选项 | 说明 |
|------|------|
| `--pause` | 创建暂停会话（自动提供智能体交互上下文） |
| `--timeout, -t <分钟>` | 会话超时时间，默认10分钟 |
| `--headed, -H` | 显示浏览器窗口（调试用） |
| `--close <sessionId>` | 关闭指定会话 |
| `--name <purpose>` | 会话用途标识 |

**RPA 脚本开发**：
详细的使用方法和完整脚本示例请参考 [AI开发RPA脚本指南](oa-todo/docs/AI开发RPA脚本指南.md)

**两种操作方式对比**：
| 方式 | 适用场景 | 优点 | 缺点 |
|------|---------|------|------|
| HTTP API | 数据查询 | 快速、高效、不依赖浏览器 | 需要找到正确的 API 端点 |
| 浏览器操作 | 页面交互 | 通用、可处理复杂交互 | 较慢、需要页面加载 |

**示例对话**：

```
用户: 帮我预约明天下午的会议室，需要容纳10人

智能体: 好的，让我为您打开会议室预约页面。

✅ 已创建会话: oa-todo-explore-meeting-1234567890
📋 页面: 会议室预约
