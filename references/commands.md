...
oa-todo rooms 20260325           # 短日期格式 (YYYYMMDD)
```
...
1. **会议室列表** - 所有会议室按楼层分组，显示座位数和设备
   - 示例：`701: 15人座 - 支持投屏`

2. **占用情况统计** - 总数、已占用、可用数量
   - 按楼层显示可用/总数
   - 完整预约列表（时间、标题、状态）
...
4. **可用时间段** - 每个会议室的空闲时段
   - 最小时段：30分钟
   - 格式：`HH:mm - HH:mm (XXX分钟)`

5. **JSON导出** - 结构化数据保存到 `/tmp/meeting_rooms_YYYYMMDD.json`

### 调试模式

使用 `--debug` 参数可查看 API 请求和响应的详细信息：

```bash
oa-todo rooms --debug              # 显示 API 请求和响应
oa-todo rooms 2026-03-25 --debug   # 指定日期 + 调试信息
```

调试输出包括：
- API 请求 URL 和 Referer
- 每页的响应状态
- 原始预约数据

### 数据来源

使用 OA 系统 HTTP API，无需打开浏览器。独立于 sync 命令，使用独立登录会话。

### 登录处理

- 自动检查登录状态，过期时自动重新登录
- 状态文件：`~/.oa-todo/login_state.json`
- 需要 `OA_USER_NAME` 和 `OA_USER_PASSWD` 环境变量
